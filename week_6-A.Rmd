...
#create tibble w/ 5 slots
q1 <- tibble(replicate = 1:5,
      
       #creates a list-column, replace = TRUE to return beads after picking them
       #use tilda when you want to apply function to object
...
q2 <- tibble(replicate = 1:1000) %>%
  ...
  #. brings in tibble, $ brings in specific
  mutate(red2 = map_int(samp, ~ sum(.$color == "red"))) %>%
  mutate(blue2 = map_int(samp, ~ sum(.$color == "blue"))) %>%
  
  #true if redcout > blue, false if otherwise
  mutate(redwins = ifelse(red2 > blue2, TRUE, FALSE)) %>%
  summarize(prob = sum(redwins)/n())
